...
class AnomalyRepository:
    ...
    def list(self, skip: int = 0, limit: int = 100) -> List[Anomaly]:
        ...
        return self.db.query(Anomaly).order_by(desc(Anomaly.created_at)).offset(skip).limit(limit).all()
    
    def list_by_status(self, status: AnomalyStatus) -> List[Anomaly]:
        """Get anomalies with specific status"""
        return self.db.query(Anomaly).filter(Anomaly.status == status.value).order_by(desc(Anomaly.created_at)).all()
    
    def list_by_assignment(self, meter_assignment_id: int) -> List[Anomaly]:
        """Get all anomalies for a meter assignment"""
        return self.db.query(Anomaly).filter(Anomaly.meter_assignment_id == meter_assignment_id).order_by(desc(Anomaly.created_at)).all()
    
    def list_by_cycle(self, cycle_id: int) -> List[Anomaly]:
        """Get all anomalies for a cycle"""
        return self.db.query(Anomaly).filter(Anomaly.cycle_id == cycle_id).order_by(desc(Anomaly.created_at)).all()
