...
class ConflictRepository:
    ...
    def list(self, skip: int = 0, limit: int = 100) -> List[Conflict]:
        ...
        return self.db.query(Conflict).order_by(desc(Conflict.created_at)).offset(skip).limit(limit).all()
    
    def list_by_status(self, status: ConflictStatus) -> List[Conflict]:
        """Get conflicts with specific status"""
        return self.db.query(Conflict).filter(Conflict.status == status.value).order_by(desc(Conflict.created_at)).all()
    
    def list_by_assignment(self, meter_assignment_id: int) -> List[Conflict]:
        """Get all conflicts for a meter assignment"""
        return self.db.query(Conflict).filter(Conflict.meter_assignment_id == meter_assignment_id).order_by(desc(Conflict.created_at)).all()
    
    def list_by_admin(self, admin_id: str) -> List[Conflict]:
        """Get conflicts assigned to a specific admin"""
        return self.db.query(Conflict).filter(Conflict.assigned_to == admin_id).order_by(Conflict.severity.desc()).all()
