...
class CycleRepository:
    ...
    def create(self, start_date: date, end_date: date, target_date: date, status: CycleStatus = CycleStatus.OPEN) -> Cycle:
        """Create a new cycle"""
        cycle = Cycle(
    ...
    def list(self, skip: int = 0, limit: int = 100) -> List[Cycle]:
        ...
        return self.db.query(Cycle).order_by(Cycle.start_date.desc()).offset(skip).limit(limit).all()
