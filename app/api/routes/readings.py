...
class SubmitReadingRequest(BaseModel):
    ...
    submitted_by: str = Field(
        ..., min_length=1, max_length=100, description="User submitting reading"
    )
    submission_notes: str = Field(
        None, max_length=500, description="Optional notes about submission"
    )
...
class ApproveReadingRequest(BaseModel):
    ...
    approved_by: str = Field(
        ..., min_length=1, max_length=100, description="Admin username"
    )
    approval_notes: str = Field(
        None, max_length=500, description="Notes about approval"
    )
    admin_consumption_override: Decimal = Field(
        None, description="Override calculated consumption (optional)"
    )
...
class VerifyRolloverRequest(BaseModel):
    ...
    max_meter_value: Decimal = Field(
        ..., gt=0, decimal_places=4, description="Meter's max value (e.g., 999999.9999)"
    )
    verified_by: str = Field(
        ...,
        min_length=1,
        max_length=100,
        description="Admin username verifying rollover",
    )
    verification_notes: str = Field(
        None, max_length=500, description="Notes about rollover verification"
    )
...
@router.post(
    "/{reading_id}/approve", response_model=ReadingRead, status_code=status.HTTP_200_OK
)
def approve_reading(
    reading_id: int,
    approved_by: str = Query(..., description="Admin username"),
    approval_notes: str = Query(None, description="Notes about approval"),
    admin_consumption_override: Decimal = Query(
        None, description="Override calculated consumption (optional)"
    ),
...
def reject_rollover(
    ...
    reading, error = service.reject_rollover_as_error(
        reading_id=reading_id, rejected_by=rejected_by, reason=reason
    )
