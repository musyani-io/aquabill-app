...
def queue_sms(sms: SMSMessageCreate, db: Session = Depends(get_db)):
...
async def send_sms(sms_id: int, db: Session = Depends(get_db)):
    """
...
def record_sent(sms_id: int, gateway_reference: str, db: Session = Depends(get_db)):
...
def record_failed(sms_id: int, error_reason: str, db: Session = Depends(get_db)):
