...
class Penalty(Base):
    ...
    status = Column(String(20), nullable=False, default=PenaltyStatus.APPLIED.value, index=True)
    imposed_by = Column(String(100), nullable=False, comment="Admin who imposed the penalty")
    imposed_at = Column(DateTime, nullable=False, server_default=func.now())
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('APPLIED', 'WAIVED')", name="ck_penalty_status_valid"),
        CheckConstraint(
