...
class SMSMessage(Base):
    ...
    sms_type = Column(String(50), nullable=False)  # "BALANCE_ALERT", "PAYMENT_REMINDER", "READING_CONFIRMATION", etc.
    
    # Related entities
    client_id = Column(Integer, ForeignKey("clients.id", ondelete="RESTRICT"), nullable=False, index=True)
    meter_assignment_id = Column(Integer, ForeignKey("meter_assignments.id", ondelete="SET NULL"), nullable=True)
    ...
    gateway_reference = Column(String(100), nullable=True, unique=True)  # ID from SMS gateway
    gateway_response = Column(Text, nullable=True)  # Full gateway response for debugging
    ...
    delivery_history = relationship("SMSDeliveryHistory", back_populates="sms_message", cascade="all, delete-orphan")
...
class SMSDeliveryHistory(Base):
    ...
    sms_message_id = Column(Integer, ForeignKey("sms_messages.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Attempt details
    attempt_number = Column(Integer, nullable=False)  # 1, 2, 3
    status = Column(SQLEnum(SMSDeliveryStatus), default=SMSDeliveryStatus.PENDING, index=True)
    ...
    callback_status = Column(String(50), nullable=True)  # "delivered", "failed", "bounced", etc.
    ...
    # Error tracking
    error_code = Column(String(50), nullable=True)  # "INVALID_NUMBER", "THROTTLED", etc.
