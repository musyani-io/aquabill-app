...
"""
import enum
from sqlalchemy import Column, Integer, String, Numeric, Boolean, DateTime, ForeignKey, CheckConstraint, func
...
class LedgerEntry(Base):
    ...
    entry_type = Column(String(20), nullable=False, default=LedgerEntryType.CHARGE.value, index=True)
    amount = Column(Numeric(12, 2), nullable=False)
    is_credit = Column(Boolean, nullable=False, default=False, comment="True for credits (payments/adjustments)")
    description = Column(String(500), nullable=False)
    created_at = Column(DateTime, nullable=False, server_default=func.now())
    created_by = Column(String(100), nullable=False, comment="Admin/user who recorded this entry")
