...
class AnomalyType(str, enum.Enum):
    ...
    NEGATIVE_CONSUMPTION = "NEGATIVE_CONSUMPTION"  # Consumption calculated as negative (rollover)
    ...
    ROLLOVER_WITHOUT_LIMIT = "ROLLOVER_WITHOUT_LIMIT"  # Rollover detected but meter max unknown
...
class Anomaly(Base):
    ...
    reading_id = Column(Integer, ForeignKey("readings.id", ondelete="SET NULL"), nullable=True)
    
    # Lifecycle
    status = Column(String(20), nullable=False, default=AnomalyStatus.DETECTED.value, index=True)
    ...
    acknowledged_by = Column(String(100), nullable=True, comment="Admin ID who acknowledged")
