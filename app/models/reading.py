...
from sqlalchemy import Column, Integer, Numeric, Date, DateTime, String, Boolean, ForeignKey, CheckConstraint, func
...
class Reading(Base):
    ...
    # Foreign keys
    meter_assignment_id = Column(Integer, ForeignKey("meter_assignments.id", ondelete="RESTRICT"), nullable=False, index=True)
    cycle_id = Column(Integer, ForeignKey("cycles.id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    consumption = Column(Numeric(9, 4), nullable=True, comment="Difference from previous reading. NULL for BASELINE")
    
    # Rollover detection
    has_rollover = Column(Boolean, default=False, comment="True if meter value wrapped/reset")
    
    # Submission tracking
    submitted_at = Column(DateTime, nullable=False, server_default=func.now())
    submitted_by = Column(String(100), nullable=False, comment="User/operator ID who submitted")
    ...
    __table_args__ = (
        ...
        CheckConstraint("consumption >= 0 OR consumption IS NULL", name="ck_reading_consumption_non_negative"),
        CheckConstraint(
            "type IN ('BASELINE', 'NORMAL')",
            name="ck_reading_type_valid"
        ),
        CheckConstraint(
