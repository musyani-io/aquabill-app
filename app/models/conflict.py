...
class ConflictType(str, enum.Enum):
    ...
    READING_ROLLOVER = "READING_ROLLOVER"  # Meter rollover detected, needs limit confirmation
    MISSING_BASELINE = "MISSING_BASELINE"  # Baseline reading missing for active assignment
...
class Conflict(Base):
    ...
    # Lifecycle
    status = Column(String(20), nullable=False, default=ConflictStatus.OPEN.value, index=True)
    
    # Detection
    created_at = Column(DateTime, nullable=False, server_default=func.now())
    
    # Assignment tracking
    assigned_to = Column(String(100), nullable=True, comment="Admin email/ID assigned to resolve")
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_conflict_severity_valid"
        ),
        CheckConstraint(
