...
class SMSService:
    ...
    def record_sent(
        self, sms_id: int, gateway_reference: str
    ) -> Optional[SMSMessageResponse]:
        """Record that SMS was sent to gateway"""
        update = SMSMessageUpdate(
            status=SMSStatus.SENT, gateway_reference=gateway_reference
        )
    ...
    def record_failed(
        self, sms_id: int, error_reason: str
    ) -> Optional[SMSMessageResponse]:
    ...
    def record_delivery_attempt(
        ...
        if db_sms:
            ...
            status = (
                SMSDeliveryStatus.SENT if not error_code else SMSDeliveryStatus.FAILED
            )
    ...
    def process_callback(
        self, gateway_reference: str, callback_status: str
    ) -> Optional[SMSMessageResponse]:
        """Process SMS gateway callback (delivered/failed/bounced)"""
        # Find SMS by gateway reference
        db_sms = (
            self.repository.db.query(SMSMessage)
            .filter(SMSMessage.gateway_reference == gateway_reference)
            .first()
        )

        if db_sms:
            return SMSMessageResponse.model_validate(
                self.repository.record_callback(
                    db_sms.id, callback_status, gateway_reference
                )
            )
        return None
    ...
    def compose_payment_confirmation(
        self, client_name: str, amount: Decimal, reference: Optional[str] = None
    ) -> str:
        """
        ...
        return f"Dear {client_name}, we received your payment of TZS {amount:,.0f}{ref_part}. Thank you."

    def compose_penalty_notice(
        self, client_name: str, amount: Decimal, reason: str
    ) -> str:
        """
        Compose penalty notice SMS.
        Example: "Dear John, a penalty of TZS 5,000 has been applied: Late payment. Thank you."
        """
        return f"Dear {client_name}, a penalty of TZS {amount:,.0f} has been applied: {reason}. Thank you."
    ...
    def create_balance_alert_sms(
        ...
        """
        message_body = self.compose_balance_alert(
            client_id, meter_assignment_id, client_name, balance, cycle_name
        )
        idempotency_key = (
            f"balance-alert-{meter_assignment_id}-{cycle_id}-{uuid.uuid4().hex[:8]}"
        )
