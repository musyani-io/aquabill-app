...
class ReadingService:
    ...
    def submit_reading(
        ...
        if cycle.status != CycleStatus.OPEN.value:
            return (
                None,
                f"Cycle {cycle_id} is not OPEN for submissions (current status: {cycle.status})",
            )
        ...
        if not existing_baseline:
            return (
                None,
                f"No baseline reading exists for assignment {meter_assignment_id}. Cannot submit normal readings.",
            )

        # ============ Check for Duplicate Submission ============
        existing_in_cycle = self.repository.get_by_assignment_and_cycle(
            meter_assignment_id, cycle_id
        )
        if existing_in_cycle:
            return (
                None,
                f"Reading already submitted for this meter in cycle {cycle_id}. ID: {existing_in_cycle.id}",
            )
        ...
        if existing_baseline:
            ...
            prev_reading = self.repository.get_latest_approved(
                meter_assignment_id, exclude_id=reading.id
            )

            if prev_reading and Decimal(absolute_value) < Decimal(
                prev_reading.absolute_value
            ):
    ...
    def calculate_consumption(
        ...
        # Get previous approved reading
        prev_reading = self.repository.get_latest_approved(
            reading.meter_assignment_id, exclude_id=reading.id
        )

        if not prev_reading:
            return (
                None,
                "No previous approved reading found for consumption calculation",
            )

        consumption = Decimal(reading.reading_value) - Decimal(
            prev_reading.reading_value
        )

        if consumption < 0:
            return (
                consumption,
                "ROLLOVER: Meter reading decreased - possible meter reset/rollover",
            )
    ...
    def approve_reading(
        ...
        if not prev_reading:
            return (
                None,
                f"No previous approved reading for meter assignment {reading.meter_assignment_id}",
            )

        # Calculate raw consumption
        raw_consumption = Decimal(reading.absolute_value) - Decimal(
            prev_reading.absolute_value
        )
        has_rollover = False
    ...
    def reject_reading(
        self, reading_id: int, rejected_by: str, rejection_reason: str
    ) -> Tuple[Optional[Reading], Optional[str]]:
        """
    ...
    def verify_rollover(
        ...
        if reading.consumption is not None and reading.consumption >= 0:
            return (
                None,
                f"Reading {reading_id} consumption is not negative (no rollover needed)",
            )
        ...
        if corrected_consumption < 0:
            return (
                None,
                f"Corrected consumption is still negative ({corrected_consumption}). Verify max_meter_value.",
            )
    ...
    def reject_rollover_as_error(
        self, reading_id: int, rejected_by: str, reason: str
    ) -> Tuple[Optional[Reading], Optional[str]]:
        """
        ...
        reading.notes = (
            f"ROLLOVER REJECTED: {rejected_by} marked as error. Reason: {reason}"
        )
        self.db.add(reading)
        self.db.commit()
        self.db.refresh(reading)
