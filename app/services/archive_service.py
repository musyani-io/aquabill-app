...
class ArchiveService:
    ...
    def archive_cycle(self, cycle_id: int) -> Tuple[bool, Optional[str]]:
        ...
        if cycle.status not in ["CLOSED"]:
            return (
                False,
                f"Cycle {cycle_id} must be CLOSED to archive (current: {cycle.status})",
            )
        ...
        if cycle.end_date > cutoff_date:
            return (
                False,
                f"Cycle {cycle_id} is not old enough (end_date: {cycle.end_date})",
            )
    ...
    def archive_old_cycles(
        self, cutoff_months: int = 36, dry_run: bool = False
    ) -> dict:
        """
    ...
    def get_archive_statistics(self) -> dict:
        ...
        """
        archived_cycles = (
            self.db.query(Cycle).filter(Cycle.status == "ARCHIVED").count()
        )

        total_cycles = self.db.query(Cycle).count()

        oldest_cycle = self.db.query(Cycle).order_by(Cycle.start_date.asc()).first()

        newest_cycle = self.db.query(Cycle).order_by(Cycle.start_date.desc()).first()
        ...
        return {
            ...
            "oldest_cycle_date": (
                oldest_cycle.start_date.strftime("%Y-%m-%d") if oldest_cycle else None
            ),
            "newest_cycle_date": (
                newest_cycle.start_date.strftime("%Y-%m-%d") if newest_cycle else None
            ),
        }
