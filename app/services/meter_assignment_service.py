...
class MeterAssignmentService:
    ...
    def assign_meter_to_client(
        self, meter_id: int, client_id: int, start_date: date, baseline_reading: float
        ...
        if existing:
            return (
                None,
                f"Meter {meter_id} already assigned to client {existing.client_id}. End that assignment first.",
            )

        # Create assignment
        create_data = MeterAssignmentCreate(
            meter_id=meter_id, client_id=client_id, start_date=start_date
        )
    ...
    def ensure_baseline_exists(self, assignment_id: int) -> tuple[bool, str | None]:
        ...
        if not baseline:
            return (
                False,
                f"Assignment {assignment_id} has no baseline reading. Cannot submit normal readings.",
            )
        return True, None
    ...
    def close_assignment(
        self, assignment_id: int, end_date: date
    ) -> MeterAssignment | None:
    ...
    def update(
        self, assignment_id: int, data: MeterAssignmentUpdate
    ) -> MeterAssignment | None:
        assignment = self.repo.get(assignment_id)
        if assignment is None:
            return None
