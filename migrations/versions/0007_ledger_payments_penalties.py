...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('entry_type', sa.String(length=20), nullable=False, server_default='CHARGE'),
        sa.Column('amount', sa.Numeric(precision=12, scale=2), nullable=False),
        sa.Column('is_credit', sa.Boolean(), nullable=False, server_default=sa.text('false'), comment='True for credits (payments/adjustments)'),
        sa.Column('description', sa.String(length=500), nullable=False),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('created_by', sa.String(length=100), nullable=False, comment='Admin/user who recorded this entry'),
        sa.ForeignKeyConstraint(['meter_assignment_id'], ['meter_assignments.id'], name='fk_ledger_meter_assignment', ondelete='RESTRICT'),
        sa.ForeignKeyConstraint(['cycle_id'], ['cycles.id'], name='fk_ledger_cycle', ondelete='RESTRICT'),
        sa.PrimaryKeyConstraint('id', name='pk_ledger_entries'),
        sa.CheckConstraint('amount >= 0', name='ck_ledger_amount_non_negative'),
        sa.CheckConstraint("entry_type IN ('CHARGE', 'ADJUSTMENT', 'PAYMENT', 'PENALTY')", name='ck_ledger_type_valid'),
    )
    op.create_index('ix_ledger_entries_id', 'ledger_entries', ['id'])
    op.create_index('ix_ledger_entries_meter_assignment_id', 'ledger_entries', ['meter_assignment_id'])
    ...
    op.create_table(
        ...
        sa.Column('method', sa.String(length=50), nullable=True, comment='cash, mobile money, bank transfer'),
        sa.Column('notes', sa.String(length=500), nullable=True),
        sa.Column('recorded_by', sa.String(length=100), nullable=False, comment='User who recorded the payment'),
        sa.Column('received_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['client_id'], ['clients.id'], name='fk_payments_client', ondelete='RESTRICT'),
        sa.ForeignKeyConstraint(['meter_assignment_id'], ['meter_assignments.id'], name='fk_payments_meter_assignment', ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['cycle_id'], ['cycles.id'], name='fk_payments_cycle', ondelete='SET NULL'),
    ...
    op.create_index('ix_payments_meter_assignment_id', 'payments', ['meter_assignment_id'])
    ...
    op.create_table(
        ...
        sa.Column('imposed_by', sa.String(length=100), nullable=False, comment='Admin who imposed the penalty'),
        sa.Column('imposed_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('waived_at', sa.DateTime(), nullable=True),
        sa.Column('waived_by', sa.String(length=100), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['meter_assignment_id'], ['meter_assignments.id'], name='fk_penalties_meter_assignment', ondelete='RESTRICT'),
        sa.ForeignKeyConstraint(['cycle_id'], ['cycles.id'], name='fk_penalties_cycle', ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id', name='pk_penalties'),
        sa.CheckConstraint('amount >= 0', name='ck_penalty_amount_non_negative'),
        sa.CheckConstraint("status IN ('APPLIED', 'WAIVED')", name='ck_penalty_status_valid'),
        sa.CheckConstraint(
    ...
    op.create_index('ix_penalties_meter_assignment_id', 'penalties', ['meter_assignment_id'])
