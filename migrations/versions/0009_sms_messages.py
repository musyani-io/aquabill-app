...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["meter_assignment_id"], ["meter_assignments.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "ix_sms_messages_idempotency_key",
        "sms_messages",
        ["idempotency_key"],
        unique=True,
    )
    ...
    op.create_index(
        "ix_sms_messages_gateway_reference",
        "sms_messages",
        ["gateway_reference"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=25), nullable=False, server_default="PENDING"
        ),
        ...
        sa.Column(
            "callback_received", sa.Boolean(), nullable=False, server_default="false"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["sms_message_id"], ["sms_messages.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "ix_sms_delivery_history_sms_message_id",
        "sms_delivery_history",
        ["sms_message_id"],
    )
    op.create_index(
        "ix_sms_delivery_history_status", "sms_delivery_history", ["status"]
    )
    op.create_index(
        "ix_sms_delivery_history_attempted_at", "sms_delivery_history", ["attempted_at"]
    )
...
def downgrade() -> None:
    """Drop SMS tables"""
    op.drop_index(
        "ix_sms_delivery_history_attempted_at", table_name="sms_delivery_history"
    )
    op.drop_index("ix_sms_delivery_history_status", table_name="sms_delivery_history")
    op.drop_index(
        "ix_sms_delivery_history_sms_message_id", table_name="sms_delivery_history"
    )
